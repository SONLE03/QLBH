...
namespace Supermarket
{
    ...
    public class DBLogic : DbConnection
    {
        ...
        public List<CostByDate> TotalCosts { get; private set; }
        ...
        private void GetCostAnalys()
        {
            TotalCosts = new List<CostByDate>();
            TotalCost = 0;
            using (var connect = GetConnection())
            {
                connect.Open();
                ...
                {
                    com.Connection = connect;
                    com.CommandText = @"select RAW_DATE, sum(RAW_TOTAL)
                                        from [RAW_MATERIAL] 
                                        where RAW_DATE between @Rstartdate and @Renddate
                                        group by RAW_DATE";
                    com.Parameters.Add("@Rstartdate", System.Data.SqlDbType.DateTime).Value = startdate;
                    com.Parameters.Add("@Renddate", System.Data.SqlDbType.DateTime).Value = enddate;

                    var read_Raw = com.ExecuteReader();
                    var resultTable_Raw = new List<KeyValuePair<DateTime, int>>();
                    while (read_Raw.Read())
                    {
                        resultTable_Raw.Add(new KeyValuePair<DateTime, int>((DateTime)read_Raw[0], (int)read_Raw[1]));
                        TotalCost += (int)read_Raw[1];
                    }
                    read_Raw.Close();
                    //Group by Hours
                    if (NumsOfDays <= 1)
                    {
                        ...
                        TotalCosts = (from orderList in resultTable_Raw
                                      group orderList by orderList.Key.ToString("hh tt")
                                        into order
                                      select new CostByDate
                                      {
                                          Date = order.Key,
                    ...
                    {
                        foreach (var item in resultTable_Raw)
                        {
                            TotalCosts.Add(new CostByDate()
                            {
        ...
        private void GetOrderAnalys()
        {
            ...
            using (var connect = GetConnection())
            {
                ...
                {
                    com.Connection = connect;
                    com.CommandText = @"select B_DATE, sum(B_PRICE), sum(TOTALAMOUNT)
                                        from BILL 
                                        where B_DATE between @startdate and @enddate
                                        group by B_DATE";
                    com.Parameters.Add("@startdate", System.Data.SqlDbType.DateTime).Value = startdate;
        ...
        private void GetProductAnalys()
        {
            ...
            using (var connect = GetConnection())
            {
                ...
                using (var com = new SqlCommand())
                {
                    ...
                    com.CommandText = @"select top 5 P.PRO_NAME, sum(BILLINFO.QUANTITY) as Q
                                        from BILLINFO
                    ...
                    //Get stock 
                    com.CommandText = "select PRO_NAME,PRO_QUANTITY from PRODUCT";
                    reader = com.ExecuteReader();
                    while (reader.Read())
                    {
                        Stock.Add(new KeyValuePair<string, int>(reader[0].ToString(), (int)reader[1]));
                    }
                    reader.Close();
                }
            }
